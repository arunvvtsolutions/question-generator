...
const ContentList: FC<ITestProps> = ({ open, chapters, baseLink, shortUrl, handleOpen }) => {
  const constructUrl = (chapter: IChapterDetailsProps, baseLink?: string, shortUrl?: string) => {
    return `/${baseLink ? `${baseLink}/` : ""}${shortUrl ? `${shortUrl}/` : ""}${chapter.Classes.shortUrl}${
    ...
    <div className="relative">
      <div
        className={`scrollbar-thin transition-all relative ${
          open ? "lg:h-max pr-2" : "lg:max-h-[200px] max-h-[265px] overflow-hidden"
        }`}
      >
      ...
      <div className="text-right mt-4">
        <button
          className={cn(`text-[#101010] hover:!text-[#0B57D0] dark:text-[#D7D7D7]`, open && "text-[#0B57D0] dark:text-[#0385FF]")}
